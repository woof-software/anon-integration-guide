...
/**
 * Repay all debt for any supported asset by Compound
 * @param param0 - chainName, account, token, repayAssetAddress
 * @param param1 - tools
 ...
 */
export async function repayWholeDept({ chainName, account, tokenAddress, repayAssetAddress }: Props, { sendTransactions, notify, getProvider }: FunctionOptions): Promise<FunctionReturn> {
    ...
    try {
        ...
        const overshootAmount = borrowedAmount / 10000n;
        const totalRepayAmount = borrowedAmount + overshootAmount;
        const amountToRepayInWei = parseUnits(totalRepayAmount.toString(), COLLATERAL_DECIMALS);
        ...
        return toResult(result.isMultisig ? repayMessage.message : `Successfully repaid ${totalRepayAmount} to ${cometName}. ${repayMessage.message}`);
    } catch (error) {
        return toResult(`Failed to repay dept into ${cometName}`, true);
    }
}
